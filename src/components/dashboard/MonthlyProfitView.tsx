...
import { 
  ...
  Layers, BarChart3, LineChart as LineChartIcon, Package, ArrowUpDown, X, RotateCcw
} from 'lucide-react';
import { 
...
export default function MonthlyProfitView({
  ...
  const activeFilterCount = useMemo(() => {
    let count = 0;
    if (preset !== 'last-6-months' || selectedSpecificMonth !== 'all') count++;
    if (branchFilter !== 'all') count++;
    if (cashierFilter !== 'all') count++;
    if (sortBy !== 'date-desc') count++;
    return count;
  }, [preset, selectedSpecificMonth, branchFilter, cashierFilter, sortBy]);

  const resetAllFilters = () => {
    setPreset('last-6-months');
  ...
  return (
    <div className="space-y-6">
      <div className="bg-white p-3.5 sm:p-4 rounded-2xl border border-slate-200 shadow-xs space-y-3">
        <div className="flex items-center justify-between gap-3">
          <div className="flex items-center gap-2.5 min-w-0">
            ...
            <div className="min-w-0">
              ...
              <p className="text-[10px] text-slate-400 truncate">
                {branchFilter !== 'all' ? (branches.find(b => b.id === branchFilter)?.name || 'Branch') : 'All Branches'}
                {' • '}
                {cashierFilter !== 'all' ? (cashiers.find(c => c.id === cashierFilter)?.name || 'Cashier') : 'All Cashiers'}
              </p>
            </div>
          </div>
          ...
          <div className="flex items-center gap-2 shrink-0">
            ...
            <button
              ...
              className="flex items-center gap-1.5 px-3 py-2 rounded-xl border border-slate-200 bg-white hover:bg-slate-50 text-slate-700 text-xs font-bold transition-all cursor-pointer disabled:opacity-50 disabled:cursor-not-allowed shadow-xs"
              title="Export monthly profit report to Excel"
            >
              <Download className="w-3.5 h-3.5 text-slate-600" />
              <span className="hidden sm:inline">{isExporting ? 'Exporting...' : 'Export XLSX'}</span>
            </button>
          </div>
        </div>

        {/* Active Filter Chips - easy way to find out what is applied and dismiss */}
        <div className="flex items-center gap-1.5 overflow-x-auto pt-2 border-t border-slate-100 no-scrollbar text-xs">
          ...
          <button
            type="button"
            onClick={() => setShowFilterDrawer(true)}
            className="inline-flex items-center gap-1 px-2.5 py-1 rounded-lg bg-slate-100 hover:bg-slate-200 text-slate-800 text-[11px] font-bold shrink-0 transition-colors cursor-pointer"
          >
          ...
          {branchFilter !== 'all' && (
            <span className="inline-flex items-center gap-1 px-2.5 py-1 rounded-lg bg-slate-100 text-slate-800 text-[11px] font-bold shrink-0">
          ...
          {selectedSpecificMonth !== 'all' && (
            <span className="inline-flex items-center gap-1 px-2.5 py-1 rounded-lg bg-slate-100 text-slate-800 text-[11px] font-bold shrink-0">
              <span>Month: {selectedSpecificMonth}</span>
              <button 
                type="button"
                onClick={() => setSelectedSpecificMonth('all')} 
                className="hover:text-black ml-0.5 cursor-pointer"
              >
                <X className="w-3 h-3" />
              </button>
          ...
          {sortBy !== 'date-desc' && (
            <span className="inline-flex items-center gap-1 px-2.5 py-1 rounded-lg bg-slate-100 text-slate-800 text-[11px] font-bold shrink-0">
              ...
              <span>
                {sortBy === 'profit-desc' ? 'Highest Profit' : sortBy === 'revenue-desc' ? 'Highest Revenue' : sortBy === 'margin-desc' ? 'Highest Margin' : 'Oldest First'}
              </span>
              <button 
                type="button"
          ...
          {activeFilterCount > 0 && (
            <button
              ...
              className="inline-flex items-center gap-1 px-2 py-1 text-[11px] font-bold text-slate-500 hover:text-black transition-colors cursor-pointer shrink-0 ml-auto"
            >
      ...
      <FilterDrawer
        ...
        title="Profit Report Filters"
        subtitle="Filter by period, month, branch, cashier & sort"
      ...
      >
        <div className="space-y-4">
          ...
          <div className="space-y-2 pt-2 border-t border-slate-100">
            <label className="block text-xs font-bold text-slate-700 uppercase tracking-wider flex items-center gap-1">
              <ArrowUpDown className="w-3.5 h-3.5 text-slate-400" /> Sort Table By
            </label>
            <select
              value={sortBy}
